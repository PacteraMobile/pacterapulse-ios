...
{
    [super setUp];
    self.testVoteContentView = [[PPLVoteContentView alloc]
        initViewWithFrame:CGRectMake(0, 0, 400, 650)];
...
- (void)testVoteContentQuestionView
{
    ...
    PPLVoteContentQuestionView *questionView =
    ...
    XCTAssertNotNil(questionView,
                    @"it should init a queationView successfully");
}
...
- (void)testVoteSlideView
{
    ...
    for (UIView *view in testSubViews)
    {
        NSString *subviewClass =
            [NSString stringWithFormat:@"%@", [view class]];
        NSString *slideClass =
            [NSString stringWithFormat:@"%@", [UISlider class]];
